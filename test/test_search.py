...
class Tests(unittest.TestCase):
    ...
    search = Search()
    filtered_dict = search.resultsTop10(search_word)
    self.assertTrue(any("Hero" in title for title in filtered_dict), "Expected a movie with 'Fast & Furious' in the title")

    def testSearchWithNumbersAndLetters(self):
    search_word = "Superman"
    search = Search()
    filtered_dict = search.resultsTop10(search_word)
    self.assertTrue(any("Superman" in title for title in filtered_dict), "Expected a movie with 'Fast & Furious' in the title")
